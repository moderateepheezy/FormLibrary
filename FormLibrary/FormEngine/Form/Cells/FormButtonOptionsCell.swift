...
public class FormButtonOptionsCell: UICollectionViewCell, FormCell {
    ...
    private func createButton(_ index: Int, selected: Bool) -> UIButton {
        ...
        button.titleLabel?.font = CellStyle.shared.fieldEntryFont
        button.tag = index
        button.addTarget(self, action: #selector(buttonPressed(_:)), for: .touchUpInside)
        button.clipsToBounds = true
        button.titleLabel?.font = CellStyle.shared.buttonLabelFont
        button.layer.cornerRadius = CellStyle.shared.fieldCornerRadius
        button.layer.borderWidth = CellStyle.shared.fieldBorderWidth
        button.layer.borderColor = CellStyle.shared.fieldBorderColor.cgColor
        if selected {
            button.backgroundColor = CellStyle.shared.fieldBorderColor
            button.setTitleColor(.white, for: .normal)
        } else {
            button.backgroundColor = .white
            button.setTitleColor(CellStyle.shared.buttonLabelColor, for: .normal)
        }
    ...
    @objc private func buttonPressed(_ sender: UIButton) {
        ...
        if self.multiSelect {
            ...
            delegate?.formButtonOptionsCell(self, selectedOptionIndex: sender.tag, stringAtIndex: sender.currentTitle ?? "")
        } else {
